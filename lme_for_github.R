...
#################################################################################################
# Load dataset
load("A:/Datasets/df_long.RData")

# Load functions
source("I:/Functions/predict_plot_function.R")


# Order datast by subject id and time to event
...
save(fit_lme, 
     file = "Z:/PROJECTS/lme model.Rdata")


# check residuals -------
plot_residuals(fitted_model = fit_lme, level = 0) # marginal
plot_residuals(fitted_model = fit_lme, level = 1) # subjects
...
hist(dataset$time_to_event)
# CHS works limitied at ten years
...
# create and save predictions (predictions occur here for mixed model as boundary knots variable is required) ------
df_predictions <- predictions_for_plot(model = fit_lme,
                                       ...
                                       plot_age = 69)
save(df_predictions, 
     file = paste0("Z:/PROJECTS/prediction.Rdata"))
